"use client";
import React from "react";

const TheTutors = ({ tutor, i }: any) => {
  return (
    <div className="tutor-card-item">
      {/* Banner with image background */}
      <div
        className="tutor-banner-section"
        style={{
          backgroundImage: `url(${
            i % 2 === 0 ? "/images/tutors/p1.png" : "/images/tutors/p2.png"
          })`,
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}>
        {/* Profile Image */}
        {/* <div className="tutor-profile-container">
          <div className="tutor-profile-wrapper">
            <img
              src={tutor?.image}
              alt={tutor?.name}
              className="tutor-profile-image"
            />
          </div>
        </div> */}
      </div>

      {/* Card Content */}
      <div className="tutor-card-content">
        <div className="tutor-name-row">
          <h3 className="tutor-name">{tutor?.name}</h3>
        </div>

        <p className="tutor-qualification">{tutor?.degree}</p>
        <p className="tutor-desc">
          Experienced physics tutor helping students master concepts and excel
          in exams.
        </p>

        <div className="tutor-subsubsub">
          <img src="/images/tutors/sub1.png" alt="Icon" />
          {tutor?.tags?.map((item: string, index: number) => (
            <span
              className={`tutor-subsub ${
                index === 0 ? "" : "tutor-subsubbbb"
              } `}
              key={index}>
              {index > 0 && <span className="subsubsub-dot"></span>} {item}
            </span>
          ))}
        </div>
        <div style={{ marginTop: "0.55rem" }} className="tutor-subsubsub">
          <img src="/images/tutors/sub2.png" alt="Icon" />
          <span className={`tutor-subsub`}>
            Personalized approach for each student
          </span>
        </div>
        <div className="tutor-teaching-badge">Interactive & Practical</div>

        <div className="tutor-stats">
          <div className="stat-item">
            <div className="stat-value">
              <span className="text-black text-lg">★</span>

              {tutor?.rating}
            </div>
            <p className="stat-label">rating</p>
          </div>
          <div className="stat-item">
            <div className="stat-value">${tutor?.rate}/hr</div>
            <p className="stat-label">rate</p>
          </div>
          <div className="stat-item">
            <div className="stat-value">{tutor?.experience}</div>
            <p className="stat-label">experience</p>
          </div>
        </div>

        <button className="booking-button">Book a Session Now</button>
      </div>
    </div>
  );
};

export default TheTutors;
