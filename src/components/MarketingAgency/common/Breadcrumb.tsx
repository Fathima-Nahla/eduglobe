...
interface BreadcrumbProps {
  ...
  subTitle: string;
  pageName: string;
  subjectName?: string; // optional, default "SUBJECTS"
  subjectLink?: string; // optional, default "/"
}

const Breadcrumb:React.FC<BreadcrumbProps> = ({ title, subTitle, pageName,subjectName,
  subjectLink, }) => {
  const finalTitle = title?.trim() || "TITLE";
  const finalSubTitle = subTitle?.trim() || "Sub Title";
  const finalPageName = pageName?.trim() || "Page Name";
  const finalSubjectName = subjectName?.trim() || "SUBJECTS";
const finalSubjectLink = subjectLink?.trim() || "/subjects-we-offer";
          ...
          <div className="section-content-wrapper">
            ...
            <div className="section-content">
              <div className="section-title-wrapper">
                <div className="title-wrapper">
                  <h1 className="section-title char-anim" suppressHydrationWarning={true}>
                    {finalTitle}
                  </h1>
                </div>
            ...
            >
              <ul className="breadcrumb">
                <li>
                  <li>
  <Link href={finalSubjectLink}>{finalSubjectName}</Link>
</li>
